...
var LambdaMapperInstance = NewLambdaMapper()

func (m *LambdaMapper) New() *Lambda {
	return &Lambda{}
}

func (m *LambdaMapper) ResourceIdentity() abs.ResourceIdentity {
...
func (m *LambdaMapper) ToRecord(lambda *Lambda) *model.Record {
	var rec = &model.Record{}
	rec.Properties = m.ToProperties(lambda)

	if lambda.Id != nil {
		rec.Id = lambda.Id.String()
	}

	return rec
}

func (m *LambdaMapper) FromRecord(record *model.Record) *Lambda {
	return m.FromProperties(record.Properties)
}

func (m *LambdaMapper) ToProperties(lambda *Lambda) map[string]*structpb.Value {
	...
	var_EventSelectorPattern := lambda.EventSelectorPattern

	var var_EventSelectorPattern_mapped *structpb.Value

	var var_EventSelectorPattern_err error
	var_EventSelectorPattern_mapped, var_EventSelectorPattern_err = types.ByResourcePropertyType(model.ResourceProperty_STRING).Pack(var_EventSelectorPattern)
	if var_EventSelectorPattern_err != nil {
		panic(var_EventSelectorPattern_err)
	}
	properties["eventSelectorPattern"] = var_EventSelectorPattern_mapped

	var_Function := lambda.Function
	...
	var_Annotations := lambda.Annotations
	...
	var_CreatedBy := lambda.CreatedBy
	...
	var_UpdatedBy := lambda.UpdatedBy
	...
	var_CreatedOn := lambda.CreatedOn
	...
	var_UpdatedOn := lambda.UpdatedOn
...
func (m *LambdaMapper) FromProperties(properties map[string]*structpb.Value) *Lambda {
	var s = m.New()
	...
	if properties["eventSelectorPattern"] != nil && properties["eventSelectorPattern"].AsInterface() != nil {

		var_EventSelectorPattern := properties["eventSelectorPattern"]
		val, err := types.ByResourcePropertyType(model.ResourceProperty_STRING).UnPack(var_EventSelectorPattern)

		if err != nil {
			panic(err)
		}

		var_EventSelectorPattern_mapped := val.(string)

		s.EventSelectorPattern = var_EventSelectorPattern_mapped
	}
